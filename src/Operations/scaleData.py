...
# check cached sclaed data or load it again
if os.path.exists(scalerLoc) and os.path.exists(x_train_scaledLoc) and os.path.exists(x_test_scaledLoc):
    X_train_scaled = np.load(x_train_scaledLoc
)
    X_test_scaled  = np.load(x_test_scaledLoc)
    with open(scalerLoc, "rb") as f:
